import prisma from "./index"
import { StageType, QuestionType, SlideType } from "./prisma/generated/client"

const questionsData: Record<string, any[]> = {
  sulawesi: [
    {
      id: 1,
      question: "Siapakah tokoh yang ditakuti oleh anak-anak dalam cerita ini?",
      options: [
        "A. Nenek Pakande",
        "B. Nenek Rara",
        "C. Putri Kemuning",
        "D. Mak Lampir",
      ],
      correctAnswer: 0,
    },
    {
      id: 2,
      question: "Di mana anak-anak biasanya bermain sebelum diculik?",
      options: ["A. Di hutan", "B. Di lapangan", "C. Di sungai", "D. Di kebun"],
      correctAnswer: 1,
    },
    {
      id: 3,
      question: "Mengapa anak-anak dilarang bermain terlalu jauh dari rumah?",
      options: [
        "A. Karena hari sangat panas",
        "B. Karena takut bertemu Nenek Pakande",
        "C. Karena mereka harus membantu memasak",
        "D. Karena banyak hewan liar",
      ],
      correctAnswer: 1,
    },
    {
      id: 4,
      question: "Bagaimana ciri khas Nenek Pakande dalam cerita?",
      options: [
        "A. Berpakaian rapi dan wangi",
        "B. Baik hati kepada anak-anak",
        "C. Berpenampilan seram dan membawa keranjang",
        "D. Suka memberi hadiah",
      ],
      correctAnswer: 2,
    },
    {
      id: 5,
      question: "Mengapa anak-anak mudah ditipu oleh Nenek Pakande?",
      options: [
        "A. Karena mereka lapar",
        "B. Karena mereka tidak mengenalnya",
        "C. Karena mereka mudah percaya pada orang asing",
        "D. Karena mereka ingin bermain lebih lama",
      ],
      correctAnswer: 2,
    },
    {
      id: 6,
      question:
        "Apa yang terjadi pada anak-anak yang terlalu jauh bermain dari desa?",
      options: [
        "A. Mereka disuruh pulang oleh orang tua",
        "B. Mereka diajak piknik",
        "C. Mereka ditemukan pedagang",
        "D. Mereka berisiko dibawa oleh Nenek Pakande",
      ],
      correctAnswer: 3,
    },
    {
      id: 7,
      question: "Bagaimana reaksi warga saat mengetahui anak-anak hilang?",
      options: [
        "A. Biasa saja",
        "B. Mereka marah dan hanya menunggu",
        "C. Mereka panik dan langsung mencari",
        "D. Mereka menyuruh anak-anak lain mencari",
      ],
      correctAnswer: 2,
    },
    {
      id: 8,
      question:
        "Apa yang mungkin dirasakan anak-anak ketika bertemu Nenek Pakande?",
      options: ["A. Senang", "B. Takut", "C. Bangga", "D. Penasaran"],
      correctAnswer: 1,
    },
    {
      id: 9,
      question: "Mengapa cerita ini sering diceritakan orang tua?",
      options: [
        "A. Untuk menakut-nakuti agar tidak bermain jauh",
        "B. Untuk hiburan",
        "C. Untuk belajar budaya makan",
        "D. Untuk belajar menghitung",
      ],
      correctAnswer: 0,
    },
    {
      id: 10,
      question: "Apa tema utama dari cerita Nenek Pakande?",
      options: [
        "A. Petualangan dan keberanian",
        "B. Persahabatan dan kerja sama",
        "C. Kedisiplinan dan kepatuhan kepada orang tua",
        "D. Cara mengalahkan raksasa",
      ],
      correctAnswer: 2,
    },
  ],

  sumatra: [
    {
      id: 1,
      question: "Di mana Malin Kundang tinggal bersama ibunya?",
      options: [
        "Di pegunungan Jawa",
        "Di kampung nelayan pesisir Sumatera Barat",
        "Di tengah hutan",
        "Di kota besar",
      ],
      correctAnswer: 1,
    },
    {
      id: 2,
      question: "Bagaimana sifat Malin Kundang saat kecil?",
      options: [
        "Pemalas dan kasar",
        "Sombong dan keras kepala",
        "Rajin dan patuh",
        "Pendiam dan penakut",
      ],
      correctAnswer: 2,
    },
    {
      id: 3,
      question: "Apa pekerjaan ibu Malin Kundang?",
      options: [
        "Pedagang kaya",
        "Janda miskin yang bekerja keras",
        "Istri bangsawan",
        "Pemilik kapal",
      ],
      correctAnswer: 1,
    },
    {
      id: 4,
      question: "Mengapa Malin ingin merantau?",
      options: [
        "Ia dimarahi ibunya",
        "Ia ingin menjadi bangsawan",
        "Ia tidak puas dengan kehidupan miskin",
        "Ia ingin meninggalkan ibunya",
      ],
      correctAnswer: 2,
    },
    {
      id: 5,
      question: "Apa pesan terakhir ibu Malin sebelum ia pergi?",
      options: [
        "Jangan kembali ke kampung",
        "Carilah harta sebanyak mungkin",
        "Jangan melupakan ibumu",
        "Jangan menikah",
      ],
      correctAnswer: 2,
    },
    {
      id: 6,
      question: "Apa yang terjadi setelah Malin sukses di perantauan?",
      options: [
        "Ia kembali miskin",
        "Ia menikah dan memiliki kapal besar",
        "Ia melupakan pekerjaannya",
        "Ia tinggal bersama ibunya",
      ],
      correctAnswer: 1,
    },
    {
      id: 7,
      question: "Bagaimana sikap Malin saat bertemu ibunya di pelabuhan?",
      options: [
        "Memeluk ibunya dengan bahagia",
        "Menangis dan meminta maaf",
        "Mengingkari dan mengusir ibunya",
        "Tidak mengenali ibunya",
      ],
      correctAnswer: 2,
    },
    {
      id: 8,
      question: "Mengapa ibu Malin berdoa kepada Tuhan?",
      options: [
        "Karena ingin kaya",
        "Karena merasa marah",
        "Karena hatinya sangat terluka oleh sikap Malin",
        "Karena ingin pindah desa",
      ],
      correctAnswer: 2,
    },
    {
      id: 9,
      question: "Apa yang menyebabkan Malin dikutuk menjadi batu?",
      options: [
        "Ia kehilangan kapalnya",
        "Ia tidak pulang ke kampung",
        "Ia durhaka dan sombong kepada ibunya",
        "Ia tidak berdoa",
      ],
      correctAnswer: 2,
    },
    {
      id: 10,
      question: "Pesan moral utama dari legenda Malin Kundang adalah …",
      options: [
        "Jangan merantau jauh",
        "Kekayaan membawa kebahagiaan",
        "Hormati orang tua dan jangan sombong",
        "Jangan tinggal di pesisir",
      ],
      correctAnswer: 2,
    },
  ],

  jawa: [
    {
      id: 1,
      question: "Di kerajaan mana Roro Jonggrang tinggal?",
      options: [
        "Kerajaan Majapahit",
        "Kerajaan Prambanan",
        "Kerajaan Pengging",
        "Kerajaan Mataram",
      ],
      correctAnswer: 1,
    },
    {
      id: 2,
      question: "Bagaimana sifat Roro Jonggrang menurut cerita?",
      options: [
        "Pemarah dan sombong",
        "Licik dan jahat",
        "Baik hati dan sopan",
        "Pendiam dan penakut",
      ],
      correctAnswer: 2,
    },
    {
      id: 3,
      question: "Siapakah Bandung Bondowoso?",
      options: [
        "Raja Prambanan",
        "Pangeran dari Majapahit",
        "Ksatria sakti dari Kerajaan Pengging",
        "Rakyat biasa",
      ],
      correctAnswer: 2,
    },
    {
      id: 4,
      question:
        "Mengapa Roro Jonggrang tidak langsung menolak lamaran Bandung Bondowoso?",
      options: [
        "Karena ia mencintainya",
        "Karena ia ingin menjadi ratu",
        "Karena takut pada kemarahan Bandung Bondowoso",
        "Karena disuruh rakyat",
      ],
      correctAnswer: 2,
    },
    {
      id: 5,
      question: "Apa syarat yang diajukan Roro Jonggrang untuk menikah?",
      options: [
        "Membangun istana baru",
        "Membangun seribu candi dalam satu malam",
        "Mengalahkan kerajaan lain",
        "Memberi emas dan harta",
      ],
      correctAnswer: 1,
    },
    {
      id: 6,
      question: "Siapa yang membantu Bandung Bondowoso membangun candi?",
      options: ["Para prajurit", "Rakyat Prambanan", "Para jin", "Para raja"],
      correctAnswer: 2,
    },
    {
      id: 7,
      question:
        "Apa yang dilakukan Roro Jonggrang untuk menggagalkan pembangunan candi?",
      options: [
        "Merusak candi",
        "Menyuruh wanita menumbuk padi dan menyalakan api",
        "Melarikan diri",
        "Menyerang para jin",
      ],
      correctAnswer: 1,
    },
    {
      id: 8,
      question: "Mengapa para jin menghentikan pekerjaannya?",
      options: [
        "Karena kelelahan",
        "Karena diperintah Bandung Bondowoso",
        "Karena mengira pagi sudah tiba",
        "Karena takut pada Roro Jonggrang",
      ],
      correctAnswer: 2,
    },
    {
      id: 9,
      question: "Berapa jumlah candi yang berhasil dibangun?",
      options: ["1000", "998", "999", "1001"],
      correctAnswer: 2,
    },
    {
      id: 10,
      question: "Apa akibat dari kemarahan Bandung Bondowoso?",
      options: [
        "Roro Jonggrang diusir",
        "Kerajaan hancur",
        "Roro Jonggrang dikutuk menjadi patung",
        "Bandung Bondowoso pergi",
      ],
      correctAnswer: 2,
    },
  ],

  papua: [
    {
      id: 1,
      question: "Siapakah tokoh utama dalam cerita ini?",
      options: ["Kepala Suku", "Biwar", "Naga Raksasa", "Ibu Biwar"],
      correctAnswer: 1,
    },
    {
      id: 2,
      question: "Di mana Biwar tinggal?",
      options: [
        "Di istana kerajaan",
        "Di kota besar",
        "Di kampung kecil di tepi hutan",
        "Di atas gunung",
      ],
      correctAnswer: 2,
    },
    {
      id: 3,
      question: "Makhluk apa yang mengganggu kehidupan warga kampung?",
      options: ["Harimau", "Raksasa", "Naga", "Buaya"],
      correctAnswer: 2,
    },
    {
      id: 4,
      question: "Apa yang sering dilakukan naga terhadap warga kampung?",
      options: [
        "Menjaga ladang",
        "Merusak ladang dan memakan ternak",
        "Membantu berburu",
        "Mengajari warga bertani",
      ],
      correctAnswer: 1,
    },
    {
      id: 5,
      question: "Mengapa Biwar memutuskan melawan naga?",
      options: [
        "Ingin terkenal",
        "Ingin balas dendam",
        "Ingin melindungi kampung dan warganya",
        "Disuruh naga",
      ],
      correctAnswer: 2,
    },
    {
      id: 6,
      question: "Senjata apa saja yang dibawa Biwar?",
      options: [
        "Pedang dan perisai",
        "Tombak, busur, dan panah",
        "Pisau dan kapak",
        "Senapan",
      ],
      correctAnswer: 1,
    },
    {
      id: 7,
      question:
        "Bagian tubuh naga mana yang pertama kali diserang Biwar dengan panah?",
      options: ["Sayap", "Leher", "Mata", "Dada"],
      correctAnswer: 2,
    },
    {
      id: 8,
      question: "Bagaimana Biwar menghindari serangan api naga?",
      options: [
        "Melompat ke sungai",
        "Berlari menjauh",
        "Bersembunyi di balik batu",
        "Menyerang balik",
      ],
      correctAnswer: 2,
    },
    {
      id: 9,
      question: "Bagaimana Biwar mengalahkan naga untuk terakhir kalinya?",
      options: [
        "Memenggal kepalanya",
        "Menusuk jantungnya dengan tombak",
        "Menjatuhkannya ke jurang",
        "Membakarnya",
      ],
      correctAnswer: 1,
    },
    {
      id: 10,
      question: "Apa gelar yang diberikan kepada Biwar setelah kemenangan itu?",
      options: [
        "Biwar Sang Raja",
        "Biwar Sang Pemberani",
        "Biwar Sang Penjaga Hutan",
        "Biwar Sang Penakluk Naga",
      ],
      correctAnswer: 3,
    },
  ],
}

async function main() {
  console.log("Start seeding questions...")

  const islands = await prisma.island.findMany()

  for (const island of islands) {
    const islandKey = island.islandName.toLowerCase()
    const questions = questionsData[islandKey]

    if (!questions) {
      console.log(`No questions found for island: ${island.islandName}`)
      continue
    }

    console.log(`Processing questions for island: ${island.islandName}`)

    // Find Pre-Test and Post-Test stories
    const stories = await prisma.story.findMany({
      where: {
        islandId: island.id,
        title: {
          in: ["Pre-Test", "Post-Test"],
        },
      },
    })

    for (const story of stories) {
      const isPreTest = story.title.includes("Pre-Test")
      const stageType = isPreTest ? StageType.PRE_TEST : StageType.POST_TEST

      console.log(
        `  Seeding questions for story: ${story.title} (${stageType})`
      )

      // Delete existing interactive slides for this story to avoid duplicates
      await prisma.interactiveSlide.deleteMany({
        where: { storyId: story.id },
      })
      console.log(`    Deleted existing interactive slides`)

      // Delete existing questions for this story and stage type to avoid duplicates
      await prisma.question.deleteMany({
        where: {
          storyId: story.id,
          stageType: stageType,
        },
      })

      // Track created questions for interactive slides
      const createdQuestions: { id: string; slideNumber: number }[] = []

      for (const q of questions) {
        const createdQuestion = await prisma.question.create({
          data: {
            storyId: story.id,
            stageType: stageType,
            questionType: QuestionType.MCQ,
            questionText: q.question,
            xpValue: 10, // Default XP value
            answerOptions: {
              create: q.options.map((opt: string, index: number) => ({
                optionText: opt,
                isCorrect: index === q.correctAnswer,
              })),
            },
          },
        })

        // Use the id from questionsData as the slideNumber
        createdQuestions.push({
          id: createdQuestion.id,
          slideNumber: q.id,
        })

        console.log(`    Created question: ${q.question.substring(0, 30)}...`)
      }

      // Create interactive slides for each question (GAME type)
      for (const questionData of createdQuestions) {
        await prisma.interactiveSlide.create({
          data: {
            storyId: story.id,
            slideNumber: questionData.slideNumber,
            slideType: SlideType.GAME,
            questionId: questionData.id,
          },
        })
      }
      console.log(
        `    Created ${createdQuestions.length} interactive slides (GAME)`
      )

      // Add an ENDING slide at the end
      const lastSlideNumber = Math.max(
        ...createdQuestions.map((q) => q.slideNumber)
      )
      await prisma.interactiveSlide.create({
        data: {
          storyId: story.id,
          slideNumber: lastSlideNumber + 1,
          slideType: SlideType.ENDING,
          contentText: "Kuis Selesai.",
        },
      })
      console.log(`    Created ENDING slide`)
    }
  }

  console.log("Seeding questions finished.")
}

main()
  .catch((e) => {
    console.error(e)
    process.exit(1)
  })
  .finally(async () => {
    await prisma.$disconnect()
  })
